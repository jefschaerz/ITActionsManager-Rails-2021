class Intervention < ApplicationRecord
  ...
  scope :search_by_device, ->(query) {where('device_id = ?', "#{query}") }
  scope :search_by_status, ->(query) {where('intervention_state_id = ?', "#{query}") }
  scope :search_by_intervention_type, ->(query) {where('intervention_type_id = ?', "#{query}") }
  scope :search_by_user, ->(query) {where('user_id = ?', "#{query}") }
